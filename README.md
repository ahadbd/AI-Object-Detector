...
This is considered "Supervised Learning" because we provided the computer with *labeled data*. By explicitly creating classes (categories) and providing the names for the objects (e.g., "Bottle", "Phone") along with the example images, we "supervised" the training process. The model learned to map the input images to the specific labels we provided.
